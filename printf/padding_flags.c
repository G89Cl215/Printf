...
/*   Updated: 2019/02/06 19:39:49 by baavril          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_padding_flags4(t_pattern *voyager, t_list *vonc)
{
	...
	&& (!(voyager->conv & (i << (TYPE_START + ft_indice('%', KNOWN_CONV)))))))))
		...
		&& voyager->field_width < 0)
			ft_padding_negative_fw(voyager, vonc);
	}
	return (1);
}
...
int		ft_padding_flags2(t_pattern *voyager, t_list *vonc)
{
	...
	if (voyager->conv & (j << (ft_indice('-', KNOWN_FLAG) + 1)))
	{
		...
		|| (voyager->conv & (j << (ft_indice('#', KNOWN_FLAG) + 1))
		&& voyager->precision == ft_strlen((char*)(vonc->content))
	...
	if (voyager->conv & (j << (ft_indice('+', KNOWN_FLAG) + 1)))
	{
		...
		if (voyager->precision >= ft_strlen((char*)(vonc->content))
		&& *((char*)(vonc->content)) == '-')
